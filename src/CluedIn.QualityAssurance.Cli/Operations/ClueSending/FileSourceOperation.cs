...
{
    private const string AutoGeneratedOriginEntityKey = "--auto-generated--";
    private const string AutoGeneratedEntityCodeOriginKey = "auto-generated";
    ...
    private static readonly Regex VocabularyKeyRegex = new(@"{{VocabularyKey\[([a-zA-Z0-9\.]+)\].(Id|Name|FullName)}}");
    ...
    private async Task<(CustomMappingOptions MappingOptions, string json)> GetCustomizationsAsync(FileSource fileSource)
    {
        if (TestFileHelper.TryGetCustomizationFileStream(fileSource.UploadFilePath, out var customizationFileStream))
        {
            ...
            var serializeOptions = GetSerializerOptions();
            ...
            if (customMapping != null)
            {
                ...
                if (deserialized != null)
                {
                    ...
                    deserialized.CustomRequests = requests;
    ...
    protected async Task AddMappingOperationsAsync(List<SetupOperation> operations, FileSource fileSource, CancellationToken cancellationToken)
    {
        ...
        var (customMapping, customizationFileBody) = await GetCustomizationsAsync(fileSource).ConfigureAwait(false);
        ...
        if (!string.IsNullOrWhiteSpace(customMapping.Origin))
        {
            operations.Add(CreateSetupOperation(fileSource, customMapping.Origin, SetOriginAsync));
        }
        if (!string.IsNullOrWhiteSpace(customMapping.NameKey))
        {
            operations.Add(CreateSetupOperation(fileSource, customMapping.NameKey, SetNameKeyAsync));
        }
    ...
    {
        foreach (var entityEdge in customMapping.Edges)
        {
    ...
    private void AddCustomRequestOperations(List<SetupOperation> operations, FileSource fileSource, CustomMappingOptions customMapping, string customizationFileBody)
    {
        ...
        Logger.LogInformation("Using custom requests {CustomRequests}.", customMapping.CustomRequests.Select(x => x.Name));
    ...
    protected override Task CustomizeResultAsync(SingleIterationOperationResult result, CancellationToken cancellationToken)
    {
        ...
        var datasetIds = FileSources.Select(source => source.DataSetId.ToString()).ToList();
        var queues = result.TotalMessages.Keys;
        var queuesToRemove = queues.Where(key => key.StartsWith("clue_datasource_process_") && datasetIds.All(datasetId => !key.Contains(datasetId))).ToList();

        foreach (var keyToRemove in queuesToRemove)
        {
            result.TotalMessages.Remove(keyToRemove);
            result.QueuePollingHistory.Remove(keyToRemove);
        }
    ...
    {
        foreach (var vocabularyMapping in fileSource.CustomVocabulariesMapping)
        {
            text = text.Replace($"{{{{Vocabulary[{vocabularyMapping.Key}].Id}}}}", vocabularyMapping.Value.Id.ToString());
            text = text.Replace($"{{{{Vocabulary[{vocabularyMapping.Key}].Name}}}}", vocabularyMapping.Value.Name);
            foreach (var keyMapping in vocabularyMapping.Value.KeysMapping)
            {
                text = text.Replace($"{{{{VocabularyKey[{vocabularyMapping.Key}.{keyMapping.Key}].Id}}}}", keyMapping.Value.Id.ToString());
                text = text.Replace($"{{{{VocabularyKey[{vocabularyMapping.Key}.{keyMapping.Key}].Name}}}}", keyMapping.Value.Name);
                text = text.Replace($"{{{{VocabularyKey[{vocabularyMapping.Key}.{keyMapping.Key}].FullName}}}}", $"{vocabularyMapping.Value.Name}.{keyMapping.Value.Name}");
            }
        }
    ...
    private async Task BatchCreateVocabularyKeysAsync(CustomVocabulary vocabulary, Guid vocabularyId, CustomVocabularyMappingEntry mapping, List<(Guid KeyId, string Name)> existingKeys, CancellationToken cancellationToken)
    {
        ...
        if (!keys.Any())
        {
            Logger.LogInformation("Skipping creation of keys because no new keys are found.");
            return;
        }

        var requestMessage = new HttpRequestMessage(HttpMethod.Post, requestUri)
        {
            Content = new StringContent(JsonSerializer.Serialize(keys, GetSerializerOptions()), Encoding.UTF8, ApplicationJsonContentType),
        };
    ...
    {
        var body = RequestTemplates.CreateVocabularyKeyAsync(
            vocabularyId: vocabularyId,
            vocabularyKeyName: vocabularyKey.Name,
            vocabularyKeyGroup: "Metadata",
            vocabularyKeyType: "Text"); // TODO: allow setting of type
        var response = await SendGraphQlRequestAsync(body, cancellationToken, requireAuthorization: true).ConfigureAwait(false);
    ...
    protected async Task CreateDataSourceSetAsync(FileSource fileSource, CancellationToken cancellationToken)
    {
        var body = RequestTemplates.CreateDataSourceSetAsync(
            dataSourceSetName: Path.GetFileNameWithoutExtension(fileSource.UploadFilePath),
    ...
    protected async Task AddPropertyMappingAsync(FileSource fileSource, KeyMapping keyMapping, CancellationToken cancellationToken)
    {
        ...
        var body = RequestTemplates.AddPropertyMappingAsync(
            dataSetId: fileSource.DataSetId,
            originalField: keyMapping.Field,
            useAsAlias: keyMapping.UseAsAlias.GetValueOrDefault(),
            useAsEntityCode: keyMapping.UseAsEntityCode.GetValueOrDefault(),
            vocabularyId: vocabularyId,
    ...
    {
        var body = RequestTemplates.CreateManualAnnotationAsync(
            dataSetId: fileSource.DataSetId,
            entityType: fileSource.EntityType,
            vocabularyName: fileSource.VocabularyName,
            vocabularyId: fileSource.VocabularyId);
    ...
    protected async Task AddEntityEdgeAsync(FileSource fileSource, EntityEdge entityEdge, CancellationToken cancellationToken)
    {
        ...
        var body = RequestTemplates.AddEntityEdgeAsync(
            annotationId: fileSource.AnnotationId,
            entityType: entityType,
            edgeType: entityEdge.EdgeType,
            origin: entityEdge.Origin,
            edgeDirection: entityEdge.Direction,
    ...
    protected async Task AddEntityCodeViaMappingAsync(FileSource fileSource, EntityCode entityCode, CancellationToken cancellationToken)
    {
        ...
        var body = RequestTemplates.AddEntityCode(
            annotationId: fileSource.AnnotationId,
            vocabularyKeyFullName: vocabularyKeyFullName,
    ...
    protected async Task<(string Json, IEnumerable<GraphQLError> Errors)> AddEntityCodeViaAnnotationCodeAsync(FileSource fileSource, EntityCode entityCode, CancellationToken cancellationToken)
    {
        ...
        var body = RequestTemplates.AddEntityCodeViaAnnotationCode(
            annotationId: fileSource.AnnotationId,
            vocabularyKeyFullName: vocabularyKeyFullName,
            entityCodeOrigin: entityCode.Origin,
            annotationKey,
    ...
    protected async Task SetNameKeyAsync(FileSource fileSource, string nameKey, CancellationToken cancellationToken)
    {
        ...
        var body = RequestTemplates.SetNameKeyAsync(
            annotationId: fileSource.AnnotationId,
    ...
    protected async Task SetOriginAsync(FileSource fileSource, string origin, CancellationToken cancellationToken)
    {
        var body = RequestTemplates.SetOriginAsync(
            annotationId: fileSource.AnnotationId,
            origin: origin);
        var response = await SendGraphQlRequestAsync(body, cancellationToken, requireAuthorization: true).ConfigureAwait(false);
        _ = await CheckResponse(response);
    ...
    private static (Guid VocabularyId, Guid VocabularyKeyId) GetVocabularyKeyDetails(FileSource fileSource, string fullVocabularyKey)
    {
        ...
        if (indexOfKey == -1)
        {
            var foundVocabularyKeys = VocabularyKeyRegex.Matches(fullVocabularyKey)
            .Select(match => match.Groups[1].Value)
            .Distinct()
            .ToList();

            if (foundVocabularyKeys.Count == 1)
            {
                var currentKeyParts = foundVocabularyKeys.Single().Split('.');
                var vocabularyName = string.Join(".", currentKeyParts.SkipLast(1));
                var keyName = currentKeyParts.Last();

                if (fileSource.CustomVocabulariesMapping.TryGetValue(vocabularyName, out var vocabularyMapping)
                    && vocabularyMapping.KeysMapping.TryGetValue(keyName, out var keyMapping))
                {
    ...
    protected class CustomMappingOptions
    {
        ...
        public string OriginEntityCodeKey { get; set; } = AutoGeneratedOriginEntityKey;
        ...
        public IEnumerable<EntityEdge> Edges { get; set; } = Enumerable.Empty<EntityEdge>();
